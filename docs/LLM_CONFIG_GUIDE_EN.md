...
- Some OpenAI-compatible relay or reasoning models reject the `temperature` parameter even though `/chat/completions` works without it.
- The main analyzer, market review, direct Agent LiteLLM calls, and system-settings channel connectivity/capability tests no longer pass `temperature` to LiteLLM by default.
- `LLM_TEMPERATURE` remains in the config surface for legacy `.env` compatibility, Web settings display, and backup/rollback workflows; saving or importing config will not silently remove or rewrite it.
- Repository-side compatibility coverage lives in `tests/test_market_analyzer_generate_text.py`, `tests/test_agent_pipeline.py`, and `tests/test_system_config_service.py`.
- Minimal rollback: restore the previous request behavior that sent `temperature`, or switch to a model/relay that accepts the parameter.
...
| **Relay models return `openai_error` / `invalid temperature` when `temperature` is present** | Some OpenAI-compatible relays or reasoning models reject sampling-temperature parameters. | Runtime calls no longer pass `temperature` by default. If it still fails, check the model name, Base URL, Key permissions, balance, and channel-mode consistency first. |
